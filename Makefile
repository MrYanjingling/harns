...
#    test/e2e/system/specs/. For example, "SYSTEM_SPEC_NAME=gke" will use
#    the spec at test/e2e/system/specs/gke.yaml. If unspecified, the
...
#
# Example:
#   make test-e2e FOCUS=Kubelet SKIP=container
#   make test-e2e REMOTE=true DELETE_INSTANCES=true
#   make test-e2e TEST_ARGS='--kubelet-flags="--cgroups-per-qos=true"'
# Build and run tests.
endef
.PHONY: test-e2e
ifeq ($(PRINT_HELP),y)
...
test-e2e: ginkgo e2e verify-golang
	hack/make-rules/test-e2e.sh
endif
