...
func (fc *FsClient) Create(key string, obj interface{}) (interface{}, error) {
	...
	defer f.Close()
	err = gob.NewEncoder(f).Encode(obj)
	if err != nil {
...
func (fc *FsClient) Delete(key, version string) (interface{}, error) {
	...
	err = gob.NewDecoder(f).Decode(&target)
	if err != nil {
...
func (fc *FsClient) Update(key, version string, obj interface{}) (interface{}, error) {
	...
	err = gob.NewDecoder(f).Decode(&old)
	if err != nil {
		klog.V(2).InfoS("Failed to unmarshal", "err", err)
		return nil, apis.ErrInternal
	}
...
func (fc *FsClient) Watch(stopCh <-chan struct{}, key string, rev string) (chan *Event, error) {
	watcher, err := fsnotify.NewWatcher()
	if err != nil {
		klog.Fatal(err)
	}

	err = watcher.Add(filepath.Join(fc.storePath, key))
	if err != nil {
		klog.Fatal(err)
	}
	...
	go func() {
		for {
			...
			case e, ok := <-watcher.Events:
				...
				_, f := filepath.Split(e.Name)
				if e.Op&fsnotify.Create == fsnotify.Create {
					klog.V(5).InfoS("Created", "file", e.Name)
					if err != nil {
						klog.V(2).InfoS("Failed to read", "name", e.Name, "err", err)
					} else {
						ch <- &Event{
							Type: Create,
							Data: f,
					...
					klog.V(5).InfoS("Updated", "file", e.Name)
					if err != nil {
						klog.V(2).InfoS("Failed to read", "name", e.Name, "err", err)
					} else {
						ch <- &Event{
					...
					klog.V(5).InfoS("Deleted", "file", e.Name)
					if err != nil {
						// TODO: need it ?
						// klog.Error(err)
					} else {
			...
			case err := <-watcher.Errors:
				klog.ErrorS(err, "Watcher")
				// TODO convert error to Event
			case <-stopCh:
