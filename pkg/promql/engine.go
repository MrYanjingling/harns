...
import (
	...
	"lightiot/pkg/promql/common/model"

	"lightiot/pkg/promql/labels"
	"lightiot/pkg/promql/parser"
	"lightiot/pkg/promql/storage"
	"lightiot/pkg/promql/value"
	"lightiot/pkg/util/stats"
...
func (ng *Engine) getTimeRangesForSelector(s *parser.EvalStmt, n *parser.VectorSelector, path []parser.Node, evalRange time.Duration) (int64, int64) {
	start, end := timeutil.FromTime(s.Start), timeutil.FromTime(s.End)
...
func (ev *evaluator) eval(expr parser.Expr) (parser.Value, storage.Warnings) {
	...
	case *parser.BinaryExpr:
		...
		case lt == parser.ValueTypeVector && rt == parser.ValueTypeScalar:
			return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (Vector, storage.Warnings) {
				// jimmy disable return bool for right operand is bool scalar
				// return ev.VectorscalarBinop(e.Op, v[0].(Vector), Scalar{V: v[1].(Vector)[0].Point.V}, false, e.ReturnBool, enh), nil
				return ev.VectorscalarBinop(e.Op, v[0].(Vector), Scalar{V: v[1].(Vector)[0].Point.V}, false, false, enh), nil
			}, e.LHS, e.RHS)
	...
	case *parser.VectorSelector:
		...
		// TODO Jimmy reset it to reuse the parsed expression with different raw data
		// Or, it will use the last time raw data in checkAndExpandSeriesSet
		e.Series = nil
		return mat, ws
...
func preprocessExprHelper(expr parser.Expr, start, end time.Time) bool {
	...
	case *parser.VectorSelector:
		if n.StartOrEnd == parser.START {
			n.Timestamp = makeInt64Pointer(timeutil.FromTime(start))
		} else if n.StartOrEnd == parser.END {
			n.Timestamp = makeInt64Pointer(timeutil.FromTime(end))
		}
	...
	case *parser.StringLiteral, *parser.NumberLiteral:
		return true
	// TODO Jimmy add this case to reuse the parsed expression. I am not sure if it is correct or not.
	case *parser.StepInvariantExpr:
		return false
	}
...
func (ng *Engine) NewInstantQuery2(q storage.Queryable, expr parser.Expr, ts time.Time) (Query, error) {
	qry, err := ng.newQuery(q, expr, ts, ts, 0)
	if err != nil {
		return nil, err
	}
