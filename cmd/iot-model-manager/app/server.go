...
import (
	...
	"k8s.io/klog/v2"
	"lightiot/cmd/iot-model-manager/options"
...
const (
	ComponentModel = "iot-model-manager"
)

func NewModelManagerCmd() *cobra.Command {
	cleanFlagSet := pflag.NewFlagSet(ComponentModel, pflag.ContinueOnError)
	o := options.NewDefaultOptions()
	cmd := &cobra.Command{
		Use:                ComponentModel,
		Long:               `The IoT model manager manages the property set type, thing type, thing, and agent.`,
...
func startServer(router *gin.Engine, o *options.Options, config *model.Config) (func(context.Context), error) {
	...
	if err := s.InstallAPIs(allowMethods, config, model.RESTProvider{}); err != nil {
		return nil, err
	}
	return s.Start(), nil
}
