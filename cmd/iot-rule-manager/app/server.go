...
import (
	...
	"k8s.io/klog/v2"
	"lightiot/cmd/iot-rule-manager/options"
...
const (
	ComponentRule = "iot-rule-manager"
)

func NewRuleManagerCmd() *cobra.Command {
	cleanFlagSet := pflag.NewFlagSet(ComponentRule, pflag.ContinueOnError)
	o := options.NewDefaultOptions()
	cmd := &cobra.Command{
		Use: ComponentRule,
		Long: `The IoT rule manager serves as a http server that receives
rules based on properties and apply them.`,
...
func run(o *options.Options) error {
	...
	exit, err := startServer(generic.Default(), o, &c.RuleConfig)
	if err != nil {
		return err
	}
...
func startServer(router *gin.Engine, o *options.Options, config *rule.Config) (func(context.Context), error) {
	...
	if err := s.InstallAPIs(allowMethods, config, rule.RESTProvider{}); err != nil {
		return nil, err
	}
	return s.Start(), nil
}
