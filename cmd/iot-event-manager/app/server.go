...
import (
	...
	"k8s.io/klog/v2"
	"lightiot/cmd/iot-event-manager/options"
	"lightiot/pkg/event"
...
const (
	ComponentEvent = "iot-event-manager"
)

func NewEventManagerCmd() *cobra.Command {
	cleanFlagSet := pflag.NewFlagSet(ComponentEvent, pflag.ContinueOnError)
	o := options.NewDefaultOptions()
	cmd := &cobra.Command{
		Use:                ComponentEvent,
		Long:               `The IoT event manager manages the event type and event.`,
...
func run(o *options.Options) error {
	...
	exit, err := startServer(generic.Default(), o, &c.EventConfig)
	if err != nil {
		return err
	}
...
func startServer(router *gin.Engine, o *options.Options, config *event.Config) (func(context.Context), error) {
	...
	if err := s.InstallAPIs(allowMethods, config, event.RESTProvider{}); err != nil {
		return nil, err
	}

	event.InstallHandlers(s.Router, config.EventMgr)
