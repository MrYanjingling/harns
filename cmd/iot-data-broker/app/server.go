...
import (
	...
	"k8s.io/klog/v2"
	"lightiot/cmd/iot-data-broker/options"
...
const (
	ComponentDataBroker = "iot-data-broker"
)

func NewDataBrokerCmd() *cobra.Command {
	cleanFlagSet := pflag.NewFlagSet(ComponentDataBroker, pflag.ContinueOnError)
	o := options.NewDefaultOptions()
	cmd := &cobra.Command{
		Use:                ComponentDataBroker,
		Long:               `The IoT data broker is a daemon that consumes the time series data from mqtt, and delivers the data to mqtt.`,
...
func run(o *options.Options) error {
	...
	exit := startServer(generic.Default(), c.ExchangeMgr, o)
...
func startServer(router *gin.Engine, m *exchange.Manager, o *options.Options) func(context.Context) {
	...
	exchange.InstallBrokerHandlers(router, m)
	...
	return func(ctx context.Context) {
		...
		if err := srv.Shutdown(ctx); err != nil {
			klog.V(1).InfoS("Failed to shutdown HTTP server", "err", err)
		}
		m.Close()
	}
}
